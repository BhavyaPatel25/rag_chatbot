...
- Memory:
  - Current memory store is an in-memory Python dictionary of deques. It is NOT multi-process or persistent. For production, replace with a centralized store (Redis, database) to support multiple workers and persistence.
- Vectorstore:
  - `vectorstore/` is persisted locally. For scalable deployments, consider using a managed vector DB or shared file system.
- Secrets:
  - Ensure API keys are stored securely (not committed), and restrict access in production environments.
- Concurrency:
  - If running multiple Uvicorn/Gunicorn workers, ensure vectorstore and memory behave correctly across processes (Chroma persistence can be sensitive to concurrent writes).
- LLM behavior:
  - The LLM prompt enforces "answer only using the provided context" style, but model behavior can vary. Add guardrails and filtering as needed.
...
- Vector store problems:
  - If the vectorstore is corrupted, delete the `vectorstore/` directory and restart; it will be re-created from `data/`.
- Missing document:
  - Make sure `data/Bhavya Patel Resume.docx` exists and is readable.
- Ollama errors:
  - Check `OLLAMA_API_KEY` and connectivity to the Ollama endpoint (host configured at `https://ollama.com` in `ollama_llm.py`).

---

## Customization ideas

- Replace in-memory session memory with Redis for persistence across instances.
- Add authentication to the API and secure endpoints.
- Support more document formats (PDF, plain text) and more data sources.
- Add streaming responses from the LLM client for a progressive UI.
...
Small project / example — feel free to submit improvements, bug fixes, or suggestions via PRs. If you plan to extend this for production use, consider adding tests, CI, and configuration for deployment.

---

## License

No license file included. Add a LICENSE if you intend to open-source this with a specific license.
